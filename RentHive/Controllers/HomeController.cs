...
namespace RentHive.Controllers
{
    public class HomeController : Controller
    {
        ...
        public async Task<IActionResult> RevenueDetails(UserDataGetter TempData, string Noti_Type, string Noti_Message)
        {
            var userData = HttpContext.Session.GetString("UserData");
            if (string.IsNullOrEmpty(userData))
            {
            ...
            string url = "https://renthive.online/Admin_API/RevenueDretails.php";
            try
            {
                using (var client = new HttpClient())
                {
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        else
                        {
                            ...
                            ViewBag.SumItem = TempData.SumItem;
                            ViewBag.SumLivingSpaces = TempData.SumLivingSpaces;
                            ViewBag.SumStorage = TempData.SumStorage;
                            ViewBag.CountItem = TempData.CountItem /*"9"*/;
                            ViewBag.CountLivingSpaces = TempData.CountLivingSpaces /*"8"*/;
                            ViewBag.CountStorage = TempData.CountStorage /* "10"*/;
                            ViewBag.Acc_Id = TempData.Acc_id;
                            return View(newuserObject);
                        }
                    }
