...
namespace RentHive.Controllers
{
    public class UserManagement : Controller
    {
        ...
        [HttpGet]
        public ActionResult Login(string Noti_Type, string Noti_Message)
        {
        ...
        [HttpPost]
        public async Task<ActionResult> ForgotPassword(UserDataGetter TempData)
        {
            // Replace with your PHP API URL hosted on 000webhost
            string url = "https://renthive.online/Admin_API/ForgotPassword.php";

            try
            {
                using (var httpClient = new HttpClient())
                {
                    string question = TempData.Question;
                    string email = TempData.Acc_Email;
                    string answer = TempData.Answer;

                    string secquestion = question + '~' + answer ;

                    if (question == "0")
                    {
                        ViewBag.Noti_Type = "error";
                        ViewBag.Noti_Message = "Please select a question.";
                    }
                    else
                    {
                        // Create a dictionary with username and password
                        ...
                        if (response.IsSuccessStatusCode)
                        {
                            ...
                            {
                                ViewBag.Noti_Type = "error";
                                ViewBag.Noti_Message = "Email does not exist.";
                            }
                            else if (userObject.response == "Incorrect")
                            {
                                ViewBag.Noti_Type = "error";
                                ViewBag.Noti_Message = "Incorrect answer.";
                            }
                            else
                            {
                                ViewBag.Noti_Type = "success";
                                ViewBag.Noti_Message = "Credentials Succuessful.";
                                return RedirectToAction("ResetPassword", "UserManagement", new { Acc_Email = email, Noti_Type = ViewBag.Noti_Type, Noti_Message = ViewBag.Noti_Message });
                            }
                        }
                        else
                        {
        ...
        [HttpGet]
        public ActionResult ResetPassword(string Acc_Email, string Noti_Type, string Noti_Message)
        {
            ViewBag.Email = Acc_Email;
            ViewBag.Noti_Type = Noti_Type;
            ViewBag.Noti_Message = Noti_Message;
            return View();
        }
        [HttpPost]
        public async Task<ActionResult> ResetPassword(UserDataGetter TempData)
        {
            // Replace with your PHP API URL hosted on 000webhost
            string url = "https://renthive.online/Admin_API/ResetPassword.php";
            ...
            try
            {
                using (var httpClient = new HttpClient())
                {
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        {
                            ViewBag.Noti_Type = "success";
                            ViewBag.Noti_Message = "Reset Successful" ;
                            return RedirectToAction("Login", "UserManagement", new { Acc_Email = email, Noti_Type = ViewBag.Noti_Type, Noti_Message = ViewBag.Noti_Message });
        ...
        public async Task<IActionResult> Profile(UserDataGetter TempData, [FromForm] IFormFile file, string Noti_Type, string Noti_Message)
        {
            ...
            if (TempData.NumHolder == 2)
            {
                ...
                try
                {
                    ...
                    //if question and answer is not null
                    if (question != "0" && answer != "")
                    {
                        ...
                        using (var httpClient = new HttpClient())
                        {
                            ...
                            //if question and answer is not null and also password is not null
                            if (Password != null)
                            {
                                // Hash the password using SHA256
                                string hashedPassword = HashSha256(Password);
                            ...
                            //else if question and answer is not null but password is null
                            else
                            {
                                var data = new Dictionary<string, string>
                                {
                    ...
                    //else if question or answer is null
                    else if(question == "0" || answer == null)
                    {
                        using (var httpClient = new HttpClient())
                        {
                            string Password = TempData.Acc_Password;

                            //if question or answer is null and also password is not null
                            if (Password != null)
                            {
                                // Hash the password using SHA256
                                string hashedPassword = HashSha256(Password);
                            ...
                            //else if question or answer is null but password is null
                            else
                            {
                                var data = new Dictionary<string, string>
                                {
